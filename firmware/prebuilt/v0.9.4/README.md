# CR11Gateway firmware v0.9.4

Tested target: ESP32-C6-DevKitC-1 v1.2 with 8 MB flash. Factory-new devices
use BDB Network Steering across Zigbee channels 11 through 26. The image is
built for a `GRB` addressable LED; a board carrying an `RGB` part needs a
source build with the matching colour order.
...
- `manifest.json` records build versions, reproducibility, offsets,
  commissioning settings, the LED colour order, and binary hashes.
